...
function ValidarCambioEstado() {
    var idPedido = document.getElementById("hfPedidoID");
    if (idPedido.value.trim() === '') {
        ...
        lblError.innerHTML = arraySearch("ME_067", deserializeString);
        lblError.style.color = "red";
        return false;
    }
    var idEstadoPedido = document.getElementById("hfEstadoPedidoID");
    if (idEstadoPedido.value != 1) {
        var lblError = document.getElementById("lblError");
        lblError.style.display = 'block';
        var leyendas = document.getElementById("hfLeyendasIdiomaActual");
        var deserializeString = JSON.parse(leyendas.value);
        //No puede actualizar un pedido cuyo estado ya ha sido actualizado.
        lblError.innerHTML = arraySearch("ME_070", deserializeString);
        lblError.style.color = "red";
        return false;
    }
            return true;
