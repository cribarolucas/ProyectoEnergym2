...
function ValidarAprCanc() {
    var idPedido = document.getElementById("hfOrdenProduccionID");
    if (idPedido.value.trim() === '') {
        var lblError = document.getElementById("lblError");
        lblError.style.display = 'block';
        var leyendas = document.getElementById("hfLeyendasIdiomaActual");
        var deserializeString = JSON.parse(leyendas.value);
        //Debe seleccionar una orden de proucción.
        lblError.innerHTML = arraySearch("ME_073", deserializeString);
        lblError.style.color = "red";
        return false;
    }
    var idEstado = document.getElementById("hfEstadoOrdenID");
    if (idEstado.value != 1) {
        var lblError = document.getElementById("lblError");
        lblError.style.display = 'block';
        var leyendas = document.getElementById("hfLeyendasIdiomaActual");
        var deserializeString = JSON.parse(leyendas.value);
        //No puede aprobar/cancelar una orden de producción que ya ha sido aprobada/cancelada
        lblError.innerHTML = arraySearch("ME_074", deserializeString);
        lblError.style.color = "red";
        return false;
    }
    return true;
