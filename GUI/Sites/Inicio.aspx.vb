Public Class Inicio
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim permisoRestaurar As Boolean
        Dim mp As MasterPage
        If Not IsPostBack Then
            If Application("BD_Corrupta") Then
                ...
                BE_Permiso.ID = 4 'Permiso Backup/Restore
                'Consulto si el usuario que inició sesión tiene permiso para restaurar la BD
                If Not IsNothing(_usuarioConectado) Then
                    If SEG_GrupoPermiso.Validar(BE_Permiso, _usuarioConectado.Perfil) Then
                        permisoRestaurar = True
                    End If
                    _usuarioConectado.Perfil.RemoveRange(0, _usuarioConectado.Perfil.Count)
                    If permisoRestaurar Then
                        _usuarioConectado.Perfil.Add(BE_Permiso)
                        mp = Me.Master
                        mp.AgregarItemsMenuPorPermiso(_usuarioConectado.Perfil)
                    End If
                    Response.Redirect("~/Sites/Mantenimiento.aspx")
                End If
            Else
                mp = Me.Master
                If Not IsNothing(_usuarioConectado) Then
